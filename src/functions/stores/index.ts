...
import { getStoresByOwnerIdController } from '../../useCases/getStoresByOwner'
...
export const handler = async (event: APIGatewayProxyEvent, context: Context, callback: Callback): Promise<any> => {
  ...
  if (event.httpMethod === 'GET' && event.resource === '/api/v1/stores') {
    return await getStoresByOwnerIdController.execute(event, callback)
  }
  if (event.httpMethod === 'PUT' && event.resource === '/api/v1/stores/{workspace}') {
